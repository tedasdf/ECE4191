...
GIMBAL_PAN = 12  # X-axis servo pin (left/right)
GIMBAL_TILT = 25  # Y-axis servo pin (up/down)
GIMBAL_CRANE = 24  # C-axis servo pin (crane up/down)

GPIO.setmode(GPIO.BCM)
GPIO.setup(GIMBAL_PAN, GPIO.OUT)
GPIO.setup(GIMBAL_TILT, GPIO.OUT)
GPIO.setup(GIMBAL_CRANE, GPIO.OUT)

pwm_pan = GPIO.PWM(GIMBAL_PAN, 50)  # 50 Hz
pwm_tilt = GPIO.PWM(GIMBAL_TILT, 50)  # 50 Hz
pwm_crane = GPIO.PWM(GIMBAL_CRANE, 50)  # 50 Hz
...
def handle_command(ctrl: MotorController, cmd: dict, client: mqtt.Client):
    ...
    angle = cmd.get("degrees", "")

    # Print every time a keypress-style command arrives
    # if t == "all" and action in ("spool", "stop", "set"):
    #     print(f"[KEYPRESS] Received command: {cmd}")
    # elif t == "vector" and action == "set":
    #     print(f"[KEYPRESS] Received vector command: {cmd}")
...
def main():
    ...
    # try:
    #     while True:
    #         time.sleep(0.1)  # keeps loop responsive
    #         now = time.time()
    #         if now - last_heartbeat >= heartbeat_interval:
    #             print("[HEARTBEAT] MQTT loop is alive")
    #             last_heartbeat = now
    ...
    def on_message(cli, _userdata, msg):
        ...
        try:
            ...
            # print("command passed to handler")
