...
import pygame
import queue
import threading  # for typing of shutdown_event
...
def publish_commands_worker(
    ...
    try:
        # Iterate until shutdown_event becomes set (sentinel=True ends the loop)
        for _ in iter(lambda: shutdown_event.is_set(), True):
            try:
                # Lightly block to avoid busy-spin, but still exit promptly on shutdown
                command = command_queue.get(timeout=0.1)
                publish_command(command, mqtt_client, topic)
            except queue.Empty:
                # No command available right now; loop will re-check shutdown_event
                pass
    finally:
        mqtt_client.loop_stop()
        mqtt_client.disconnect()
